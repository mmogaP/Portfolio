<script lang="ts">
	...
	let selectedCategory = $state('frontend');
					...
					<Card.Root class="bg-neutral-700">
						<Card.Content class="p-6">
							<div class="mb-6 flex flex-wrap gap-2">
								...
								{#each Object.keys(technologyCategories) as category (category)}
									<button
	...
	{#if mounted}
		...
		>
			<h2 class="mb-6 flex items-center text-2xl font-bold">
				<span class="mr-3"><FolderKanban /></span>
				Projects
			</h2>

			<div class="grid gap-6 md:grid-cols-2">
				{#each projects as project (project.name)}
					<Card.Root class="flex flex-col bg-neutral-700 transition-all hover:border-blue-400">
						<Card.Header>
							<Card.Title class="text-base">{project.name}</Card.Title>
							<Card.Description class="text-sm text-gray-300">{project.description}</Card.Description>
						</Card.Header>
